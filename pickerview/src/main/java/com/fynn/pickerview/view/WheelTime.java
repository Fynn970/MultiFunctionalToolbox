package com.fynn.pickerview.view;

import android.view.View;

import com.fynn.pickerview.R;
import com.fynn.pickerview.adapter.ArrayWheelAdapter;
import com.fynn.pickerview.adapter.NumericDayWheelAdapter;
import com.fynn.pickerview.adapter.NumericMinuteWheelAdapter;
import com.fynn.pickerview.adapter.NumericWheelAdapter;
import com.fynn.pickerview.listener.ISelectTimeCallback;
import com.fynn.pickerview.utils.ChinaDate;
import com.fynn.pickerview.utils.LunarCalendar;
import com.fynn.listener.OnItemSelectedListener;
import com.fynn.view.WheelView;
...
public class WheelTime {
    ...
    private void setLunar(int year, final int month, int day, boolean isLeap, int h, int m, int s) {
        ...
        wv_minutes.setAdapter(new NumericMinuteWheelAdapter(0, 59, timeInterval));
    ...
    private void setSolar(int year, final int month, int day, int h, int m, int s) {
        ...
        if (startYear == endYear && startMonth == endMonth) {
            if (list_big.contains(String.valueOf(month + 1))) {
                ...
                wv_day.setAdapter(new NumericDayWheelAdapter(startDay, endDay, year, month + 1, weekType));
            ...
            // 起始日期的天数控制
            if (list_big.contains(String.valueOf(month + 1))) {
                wv_day.setAdapter(new NumericDayWheelAdapter(startDay, 31, year, month + 1, weekType));
            } else if (list_little.contains(String.valueOf(month + 1))) {
                wv_day.setAdapter(new NumericDayWheelAdapter(startDay, 30, year, month + 1, weekType));
            } else {
                // 闰年 29，平年 28
                wv_day.setAdapter(new NumericDayWheelAdapter(startDay, leapYear ? 29 : 28, year, month + 1, weekType));
            }
            ...
            if (list_big.contains(String.valueOf(month + 1))) {
                ...
                wv_day.setAdapter(new NumericDayWheelAdapter(1, endDay, year, month + 1, weekType));
            } else if (list_little.contains(String.valueOf(month + 1))) {
                if (endDay > 30) {
                    endDay = 30;
                }
            ...
            // 判断大小月及是否闰年,用来确定"日"的数据
            if (list_big.contains(String.valueOf(month + 1))) {
                wv_day.setAdapter(new NumericDayWheelAdapter(1, 31, year, month + 1, weekType));
            } else if (list_little.contains(String.valueOf(month + 1))) {
                wv_day.setAdapter(new NumericDayWheelAdapter(1, 30, year, month + 1, weekType));
            } else {
                // 闰年 29，平年 28
                wv_day.setAdapter(new NumericDayWheelAdapter(1, leapYear ? 29 : 28, year, month + 1, weekType));
            }
        }
        wv_day.setCurrentIndexItem(day);
        ...
        setReHour(h);
        setReMinute(m);
        currentMinute = (int) wv_minutes.getAdapter().getItem(wv_minutes.getCurrentItem());
        setReSecond(s);
        ...
        wv_month.setOnItemSelectedListener(new OnItemSelectedListener() {
            ...
            public void onItemSelected(int index) {
                ...
                currentMonth = (int) wv_month.getAdapter().getItem(index);
                if (startYear == endYear) {
        ...
        wv_day.setOnItemSelectedListener(new OnItemSelectedListener() {
            @Override
            public void onItemSelected(int index) {
                currentDay = (int) wv_day.getAdapter().getItem(index);
                setReHour((Integer) wv_hours.getAdapter().getItem(wv_hours.getCurrentItem()));
                setReMinute((Integer) wv_minutes.getAdapter().getItem(wv_minutes.getCurrentItem()));
                setReSecond((Integer) wv_seconds.getAdapter().getItem(wv_seconds.getCurrentItem()));
            }
        });
        ...
        wv_hours.setOnItemSelectedListener(new OnItemSelectedListener() {
            @Override
            public void onItemSelected(int index) {
                currentHour = (int) wv_hours.getAdapter().getItem(index);
                setReMinute((Integer) wv_minutes.getAdapter().getItem(wv_minutes.getCurrentItem()));
                setReSecond((Integer) wv_seconds.getAdapter().getItem(wv_seconds.getCurrentItem()));
            }
        });
        ...
        wv_minutes.setOnItemSelectedListener(new OnItemSelectedListener() {
            @Override
            public void onItemSelected(int index) {
                currentMinute = (int) wv_minutes.getAdapter().getItem(index);
                setReSecond((Integer) wv_seconds.getAdapter().getItem(wv_seconds.getCurrentItem()));
            }
        });
        setChangedListener(wv_seconds);
    ...
    private void setReHour(int h){
        if (startYear == endYear && startMonth == endMonth && startDay == endDay) {
            wv_hours.setAdapter(new NumericWheelAdapter(startHour, endHour));
            wv_hours.setCurrentItem(h-startHour);
        } else if ((startYear == endYear && startMonth == endMonth && currentDay == startDay)||
                (currentYear == startYear && currentMonth + 1 == startMonth && currentDay == startDay)) {
            wv_hours.setAdapter(new NumericWheelAdapter(startHour, 23));
            wv_hours.setCurrentItem(h -startHour);
        } else if ((startYear == endYear && startMonth == endMonth && currentDay == endDay)||
                (currentYear == endYear && currentMonth + 1 == endMonth && currentDay == endDay)) {
    ...
    private void setReMinute(int m){
        if (startYear == endYear && startMonth == endMonth && startDay == endDay && startHour == endHour) {
            wv_minutes.setAdapter(new NumericMinuteWheelAdapter(startMinute, endMinute, timeInterval));
//            wv_minutes.setCurrentItem(m- startMinute);
        }else if ((startYear == endYear && startMonth == endMonth && startDay == endDay && currentHour == startHour)
                ||(currentYear == startYear && currentMonth + 1 == startMonth && currentDay == startDay && currentHour == startHour)){
            wv_minutes.setAdapter(new NumericMinuteWheelAdapter(startMinute, 59, timeInterval));
//            wv_minutes.setCurrentItem(m - startMinute);
        }else if ((startYear == endYear && startMonth == endMonth && startDay == endDay && currentHour == endHour)||
                (currentYear == endYear && currentMonth + 1 == endMonth && currentDay == endDay && currentHour == endHour)){
            wv_minutes.setAdapter(new NumericMinuteWheelAdapter(0, endMinute, timeInterval));
//            wv_minutes.setCurrentItem(m);
        }else{
            wv_minutes.setAdapter(new NumericMinuteWheelAdapter(0, 59, timeInterval));
//            wv_minutes.setCurrentItem(m);
        }
        wv_minutes.setCurrentIndexItem(m);
    }

    private void setReSecond(int s){
        ...
        }else if ((startYear == endYear && startMonth == endMonth && startDay == endDay && startHour == endHour && startMinute == currentMinute) ||
                (currentYear == startYear && currentMonth + 1 == startMonth && currentDay == startDay && currentHour == startHour && currentMinute == startMinute)){
            wv_seconds.setAdapter(new NumericWheelAdapter(startSecond, 59));
            wv_seconds.setCurrentItem(s- startSecond);
        }else if ((startYear == endYear && startMonth == endMonth && startDay == endDay && startHour == endHour && endMinute == currentMinute) ||
                (currentYear == endYear && currentMonth + 1 == endMonth && currentDay == endDay && currentHour == endHour && currentMinute == endMinute)){
    ...
    private void setReDay(int year_num, int monthNum, int startD, int endD, List<
            String> list_big, List<String> list_little) {
...
//        int maxItem;
        if (list_big.contains(String.valueOf(monthNum))) {
            ...
            wv_day.setAdapter(new NumericDayWheelAdapter(startD, endD, year_num, monthNum, weekType));
//            maxItem = endD;
...
//                maxItem = endD;
    ...
    public String getTime() {
        ...
        sb.append((wv_year.getAdapter().getItem(wv_year.getCurrentItem()))).append("-")
                ...
                .append((wv_seconds.getAdapter().getItem(wv_seconds.getCurrentItem())));
    ...
    public void setWeekType(List<Integer> type) {
        weekType = type;
    }
