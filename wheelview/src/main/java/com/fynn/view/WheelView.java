...
import com.fynn.interfaces.IPickerViewData;
import com.fynn.listener.LoopViewGestureListener;
import com.fynn.listener.OnItemSelectedListener;
import com.fynn.timer.InertiaTimerTask;
import com.fynn.timer.MessageHandler;
import com.fynn.timer.SmoothScrollTimerTask;
...
public class WheelView extends View {
     ...
     * @param index
     */
    public final void setCurrentIndexItem(int index){
        int item = adapter.indexOf(index);
        this.selectedItem = item;
        this.initPosition = item;
        totalScrollY = 0;//回归顶部，不然重设setCurrentItem的话位置会偏移的，就会显示出不对位置的数据
        invalidate();
    }
    ...
    public final int getCurrentDayItem() {
        // return selectedItem;
        if (adapter == null) {
            return 0;
        }
        ...
        return (int) adapter.getItem(selectedItem);
    }
